...
Initially, district codes 503,504 were missing in some tables becuase these districts are formed after state division. So, a lot of NAs were formed while performing joins. Now we gathered latest possible information for all data sets.

$\textbf{Deleting rows < 1990}$
...
$\textbf{Summary}$
```{r}
# summary(df.biophysical.landuse)
# summary(df.biophysical.length.of.growing.period)
# summary(df.biophysical.monthly.rainfall)
# summary(df.biophysical.soil.type1)
# summary(df.environment.evapotranspiration.actual)
# summary(df.environment.evapotranspiration.potential)
# summary(df.environment.precipitation)
# summary(df.environment.temperature.maximum)
# summary(df.environment.temperature.minimum)
# summary(df.inputs.fertilizer.consumption)
# summary(df.inputs.season.fertilizer.consumption)
# summary(df.inputs.wages)
# summary(df.irrigation.rice.area)
# summary(df.irrigation.sourcewise.irrigated.area)
# summary(df.rice.area.production.yield)
# summary(df.rice.farm.harvest.price)
```
...
Now handling NA values for df.irrigation.sourcewise.irrigated.area
...
Now handling NA values for df.rice.farm.harvest.price
...
Now we will handle NA values of df.input.wages
...
Now handling missing values of df.inputs.season.fertilizer.consumption
...
$\textbf{Normalization : Standard scaling method}$
...
scale() is an inbuilt function.

```{r}
# this function scales the all columns except first 3.
scaling.fn <- function(df.name){
  df.name[,4:ncol(df.name)] <- apply(df.name[,4:ncol(df.name)],MARGIN = 2,scale)
  return(df.name)
}


df.biophysical.landuse <- scaling.fn(df.biophysical.landuse)

df.biophysical.monthly.rainfall <- scaling.fn(df.biophysical.monthly.rainfall)
df.biophysical.soil.type1 <- scaling.fn(df.biophysical.soil.type1)
df.environment.evapotranspiration.actual <- scaling.fn(df.environment.evapotranspiration.actual)
df.environment.evapotranspiration.potential <- scaling.fn(df.environment.evapotranspiration.potential)
df.environment.precipitation <- scaling.fn(df.environment.precipitation)
df.environment.temperature.maximum <- scaling.fn(df.environment.temperature.maximum)
df.environment.temperature.minimum <- scaling.fn(df.environment.temperature.minimum)
df.inputs.fertilizer.consumption <- scaling.fn(df.inputs.fertilizer.consumption)
df.inputs.season.fertilizer.consumption <- scaling.fn(df.inputs.season.fertilizer.consumption)
df.inputs.wages <- scaling.fn(df.inputs.wages)

df.irrigation.sourcewise.irrigated.area <- scaling.fn(df.irrigation.sourcewise.irrigated.area)

df.rice.area.production.yield <- scaling.fn(df.rice.area.production.yield)


#these only have 4 columns in total so our custom funciton does not work. so we do it manually.


df.biophysical.length.of.growing.period$LENGTH.OF.GROWING.PERIOD.DAYS..Number. <- scale(df.biophysical.length.of.growing.period$LENGTH.OF.GROWING.PERIOD.DAYS..Number.)

df.irrigation.rice.area$RICE.IRRIGATED.AREA..1000.ha. <- scale(df.irrigation.rice.area$RICE.IRRIGATED.AREA..1000.ha.)

df.rice.farm.harvest.price$PADDY.HARVEST.PRICE..Rs.per.Quintal. <- scale(df.rice.farm.harvest.price$PADDY.HARVEST.PRICE..Rs.per.Quintal.)
...
```{r}
#data splitting.
#Here we are splitting the data to train and test where we took data from 1990 to 2010 as training data and data from 2010 as testing data, So, technically we are predicting future results.

# test.subset <- c("2011","2012","2013","2014","2015")
# values <- df.all$Year.x %in% test.subset
# data.test <- df.all[values,]
# data.train <- df.all[!values,]

# this test split did not work because - we had taken year as a factor! and when we try to test the model with new data, since our model did not see these factors before, it could not run the model and throwing error.
...
set.seed(108)
# 
data.sample <- sample(2,nrow(df.all),replace = T, prob = c(0.8,0.2))
data.train <- df.all[data.sample ==1,]
data.test <- df.all[data.sample ==2,]


model.all <- lm(PADDY.HARVEST.PRICE..Rs.per.Quintal.~., data = data.train)
#summary(model.all)
#our model.all with all predictor variables gave an Adjusted r-squared value of 0.9772
#but our fwdfit.aic model gave Adjusted R-squared vlaues of 0.9765 with only 19 predictors
...
```
Our model with AIC slection has got an Adjusted R-squared vlaues of 0.9765 which indicates a very good model.
...
plot(training.df1$PADDY.HARVEST.PRICE..Rs.per.Quintal.,training.df1$pred.reg.train,main = "Actual vs Predicted values: Train data",xlab = "Actual data",ylab = "Predicted data")
#testing predictions
prediction.test <- predict(fwdfit.aic$model,newdata = data.test)
testing.df1 <- data.test%>% mutate(pred.reg.test = prediction.test)
plot(testing.df1$PADDY.HARVEST.PRICE..Rs.per.Quintal.,testing.df1$pred.reg.test,main = "Actual vs Predicted values: Test data",xlab = "Actual data",ylab = "Predicted data")


#Calculating RMSE
RMSE.train <- sqrt(mean((training.df1$pred.reg.train-training.df1$PADDY.HARVEST.PRICE..Rs.per.Quintal.)^2))
RMSE.train
RMSE.test <- sqrt(mean((testing.df1$pred.reg.test-testing.df1$PADDY.HARVEST.PRICE..Rs.per.Quintal.)^2))
RMSE.test
```
As we can see, our model predicted almost correct results.
...
#now we will try to do model selction using lasso.
```{r}
library(glmnet)
...
prediction.test.lasso

#RMSE
RMSE.test.lasso <- sqrt(mean((data.test$PADDY.HARVEST.PRICE..Rs.per.Quintal. - prediction.test.lasso)^2))

RMSE.test.lasso
```

#Conclusion:
Our test RMSE for AIC model is  0.1710452, our test RMSE for Lasso model is  0.1547849
...
#TODO
Replace factor for Year as normal year.
rescale the restuls
