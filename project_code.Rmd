...
df.all <- full_join(combine14,df.biophysical.soil.type,by = c("Dist.Code","State.Code","State.Name","Dist.Name"))
...
#TODO
df.all$SOIL.TYPE.PERCENT..Percent. <- factor(df.all$SOIL.TYPE.PERCENT..Percent., levels = c("ORTHIDS","SANDY ALFISOL","LOAMY ALFISOLS","VERTISOLS","VERTIC SOILS")) 
#this method is not working, try to create dummy varaibles for soil type.
#also try to take average of rainfall for 3 months each so we have 4 variables instead of 12 variables.

```

Our final dataframe consists of 884 observations with 133 predictors.
...
```{r}
str(df.all)
str(df.all$SOIL.TYPE.PERCENT..Percent.)
summary(df.all)
```
