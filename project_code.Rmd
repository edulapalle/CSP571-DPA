...
df.biophysical.soil.type1 <- read.csv("/Users/santosh/Documents/CSP571/CSP571-DPA/Datasets/Biophysical_soil_type.csv")
...
Initillay, district codes 503,504 were missing in some tables becuase these districts are formed after state division. So, a lot of NAs were formed while performing joins. Now we gathered latest possible information for all data sets.
...
df.rice.fram .price table has 2 missing values for district 503,504 for year 1995. and this was not coded as -1. Instead it is completely missing.
...
#further reduce predictors
#build model - lm & random forest - lasso


finidng corelation
This below chunk is cannot give accurate results because of too many variables.
```{r}
# library(caret)
# library(corrplot)
...
# y = data.train$PADDY.HARVEST.PRICE..Rs.per.Quintal.
# x = data.train%>% dplyr::select(-c(PADDY.HARVEST.PRICE..Rs.per.Quintal.))
# x = model.matrix(x)
